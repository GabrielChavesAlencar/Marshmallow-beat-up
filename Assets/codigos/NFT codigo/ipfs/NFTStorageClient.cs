        ...
        */
        ...
        IEnumerator PostRequest(string url, string filePath)
        {
            ...
            else
            {
                ...
                NFTStorageCustomUploadResponse parseResponse = JsonUtility.FromJson<NFTStorageCustomUploadResponse>(uwr.downloadHandler.text);
                Text cidText =  Cid.GetComponent<Text>();
                cidText.text = parseResponse.value.cid;
              //  Debug.Log("CID: "+Cid);
                Debug.Log("CID: "+cidText.text);
                PlayerPrefs.SetString("json", cidText.text);
                if(ImplementNFTStorage.fase==1){
                    ImplementNFTStorage.fase = 2;
                }
                if(ImplementNFTStorage.fase==3){
                    ImplementNFTStorage.fase = 4;
                }
                print("fase "+ImplementNFTStorage.fase);
            }
        }
